...
const ProductDetail = () => {
    ...
    useEffect(() => {
    // Tìm sản phẩm dựa trên id string
    const prod = allProducts.find(p => p.id === id) // string so với string
    if (prod) {
        setProduct(prod)
        // Lọc sản phẩm liên quan cùng category, loại trừ chính nó
        setRelatedProducts(allProducts.filter(p => p.category === prod.category && p.id !== prod.id))
    }
}, [id])
